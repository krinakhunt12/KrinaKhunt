      ...
      <div className="container max-w-6xl mx-auto px-2 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 lg:mb-16 gap-6 md:gap-8">
          <div className="space-y-2 md:space-y-4">
            <h3 className="mono text-xs md:text-sm uppercase tracking-[0.3em] text-accent opacity-60">Showcase</h3>
            <h2 className="text-4xl md:text-5xl font-semibold">Featured <span className="text-accent opacity-60">Creations</span></h2>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-10 lg:gap-12">
          {filtered.map((project) => (
            <div
              key={project.id}
              className="group relative bg-black rounded-3xl overflow-hidden border border-border bg-primary"
            >
              ...
              <div className="p-6 md:p-8 lg:p-10 space-y-4 md:space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map(t => (
                    <span key={t} className="text-[10px] font-bold uppercase tracking-widest px-3 md:px-4 py-1 md:py-1.5 rounded-full border border-white/10 group-hover:border-[var(--accent-1)] transition-colors">{t}</span>
                  ))}
                </div>
              </div>
            </div>
